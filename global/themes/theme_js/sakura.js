...
const Sakura = function(selector, options) {
  ...
  const defaults = {
    ...
    delay: 150, // Delay between petals.
    colors: [
      {
        // You can add multiple colors (chosen randomly) by adding elements to the array.
        gradientColorStart: 'rgba(255, 183, 197, 1)', // Gradient color start (rgba).
        gradientColorEnd: 'rgba(255, 197, 208, 1)', // Gradient color end (rgba).
  ...
  this.createPetal = () => {
    ...
    const fallTime = randomInt(10, 15);

    // Create animations
    const animationsArr = [
      `fall ${fallTime}s linear 0s 1 forwards`,
      `fade ${fallTime}s cubic-bezier(1, 0, 1, 1) 0s 1 forwards`,
      `${blowAnimation} ${fallTime}s ease-in-out 0s infinite`,
      `${swayAnimation} ${randomInt(2, 4)}s ease-in-out 0s infinite`,
    ];
